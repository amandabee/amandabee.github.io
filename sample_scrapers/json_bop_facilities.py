""" 
To scrape facility addresses from http://www.bop.gov/locations/list.jsp I started
trying to scrape this w/Beautiful Soup, but it turns out this is all in JSON
"""
import urllib2
import json
import csv
...
for item in jsondata['Locations']:
    f.writerow(
        [item['hasFsl'], item['code'], item['contactEmail'], item['special'], item['city'], item['privateFacl'], item['nameDisplay'], item['faclTypeDescription'], item['state'], item['phoneNumber'], item['latitude'], item['type'],
         item['locationtype'], item['zipCode'], item['hasCamp'], item['complexCode'], item['address'], item['securityLevel'], item['name'], item['gender'], item['region'], item['longitude'], item['hasFdc'],  item['timeZone'], item['nameTitle']])
...
# Work from
# http://stackoverflow.com/questions/5318747/using-python-to-extract-dictionary-keys-within-a-list
